...
BASE_URL = "http://localhost:4000"  # Change this to your API base URL
...
def test_register_user():
    ...
    # """
    payload = {"username": "ab2cd", "password": "ab2cd"}  # Change username/password if needed
...
def test_login():
    ...
    # """
    payload = {"username": "ab2cd", "password": "ab2cd"}  # Change to test different login credentials
...
def test_get_products(token, expected_quantity):
    ...
    # """
    res = requests.get(f"{BASE_URL}/api/products/list", headers={"Authorization": f"Bearer {token}"})
    if res.status_code != 200:
